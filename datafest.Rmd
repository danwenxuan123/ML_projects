...
We want to find the relationship between education,income,rate of adherent population over county population.
...
We regress income vs rate

we can find the distribution of rate, income and percentage of people having above highschool diploma is skewed, we think it is better to perform a log transformation
Then we are interested in using different model
...
The distribution is right skewed from the qqplot and histogram,  
the independence is not violated.
we now consider using a log transformation on the model

```{r}
g[[5]]
```

The residual plots shows linearity.
but the residual shows a microphone shape, constant variance is violated
...
we observe the similar pattern for the other three year,thus we will do log transformation for the other three years, together with weighted variance.
...
Then we look at the education vs rate of religious population
...
We find by the qqplot, the distribution is more closer and less skewed than 
income distribution.
...
summary(mod2010)
```
By checking the four t test of the income regression,we will reject all four null hypothesis at 95% significants and claim there are a small negative relation between income and rate of religious population.
```{r eval=FALSE}
summary(my_model(test1980_)[[2]])
summary(my_model(test1990_)[[2]])
summary(my_model(test2000_)[[2]])
summary(my_model(test2010_)[[2]])
```

By checking the four t test of the education regression,we will reject all four null hypothesis at 95% significance and claim there are a small negative relation between education population and rate of religious population.
We then are interested in the relation between all three features:

```{r}
modelfull1<-lm(log(income)~highschool+rate2+workingrate,data=test1990_)
modelfull2<-lm(income~highschool*rate2+workingrate,data=test1980_)
modelfull3<-lm(income~highschool+rate2*workingrate,data=test1980_)
modelfull4<-lm(income~highschool*workingrate+rate2,data=test1980_)
summary(modelfull1)
summary(modelfull2)
summary(modelfull3)
summary(modelfull4)
...
we observe a dramatical increase in r-square, this may due to more close relationship between income and education and workforce population.
Because from the residual
```{r}
testplot1980_<-mutatedata(test1990_,modelfull1)
drawgrid(testplot1980_,'workingrate','income')[[5]]
```
